<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">

    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Antic+Slab&family=Barlow+Condensed:wght@300&display=swap" rel="stylesheet">

    <!-- tab icon -->
    <link rel="icon" href="assets/DukaKuu.png" type="image/x-icon">

    <!-- CSS only -->
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.2.0/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-gH2yIJqKdNHPEq0n4Mqa/HGKIhSkIHeL5AyhkYV8i59U5AR6csBvApHHNl/vI1Bx" crossorigin="anonymous">

    <!-- JavaScript Bundle with Popper -->
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.2.0/dist/js/bootstrap.bundle.min.js" integrity="sha384-A3rJD856KowSb7dwlZdYEkO39Gagi7vIsF0jrRAoQmDKKtQBHUuLZ9AsSv4jD4Xa" crossorigin="anonymous"></script>

    <!-- css -->
    <link rel="stylesheet" href="css/app.css">
    <title>DUKAKUU®</title>
</head>

<body>
    <nav class="app-navbar w-100 custom-dark lr jcc aic">
        <div class="logo">
            <img src="assets/DukaKuu.png" alt="" class="img-logo">
        </div>
        <div class="nav-options">
            <ul class="lr jcc aic td-none lst-none c-white">
                <li class="px-3 c-white"><a href="">NEW ARRIVALS</a></li>
                <li class="px-3 c-white"><a href="">MENSWEAR</a></li>
                <li class="px-3 c-white"><a href="">LADIESWEAR</a></li>
                <li class="px-3 c-white"><a href="">KIDS</a></li>
            </ul>
        </div>

    </nav>
</body>

</html>